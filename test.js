...
function abortAndEchoCheck(controller) {
  if (finalizedTranscript.includes("Dur")) {
    console.log("-------Aborted Word Detected-------");
    botVoiceState = STATES.AwaitingUtterance;
    controller.abort();
  }

  if (previousFinalTranscript === finalizedTranscript) {
    console.log("-------Bot Echo Detected-------");
    botVoiceState = STATES.AwaitingUtterance;
    previousFinalTranscript = "";
    controller.abort();
  }
...
async function openAIGenerateResponse(finalizedTranscript) {
  try {
    ...
    const prompt = ChatPromptTemplate.fromMessages([
      ...
      ["user", finalizedTranscript],
    ]);
    // abortAndEchoCheck(controller);
    ...
    console.log("Error in openAIGenerateResponse:", error);
  }
}
...
async function openAIGenereateAudio(modelFinalOutputText) {
  try {
    ...
    const response = await openai.audio.speech.create({
      model: "tts-1",
      voice: "alloy", //voice variable gelsin
...
function handleBotReply(text, audio) {
  ...
  previousFinalTranscript = finalizedTranscript;
  finalizedTranscript = "";
  unfinalizedTranscript = "";
  latestFinalizedWordEnd = Infinity;
  latestTimeSeen = 0;
